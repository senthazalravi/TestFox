                    ...
                    case 'testConnection':
                        ...
                        case 'fetchOllamaModels':
                            try {
                                ...
                                const ai = createAIService({ provider: AIProvider.OLLAMA, baseUrl, model: undefined });
                                let available = await ai.isAvailable();
                                if (!available) {
                                    // Try running the helper script to install/start Ollama
                                    try {
                                        const scriptPath = path.join(this._extensionUri.fsPath, 'scripts', 'ollama.py');
                                        this._panel.webview.postMessage({ command: 'connectionTest', success: false, message: 'Ollama not running — attempting to install/start Ollama...' });
                                        exec(`python "${scriptPath}"`, { cwd: this._extensionUri.fsPath }, async (err, stdout, stderr) => {
                                            if (err) {
                                                this._panel.webview.postMessage({ command: 'ollamaModels', success: false, message: `Failed to run Ollama script: ${err.message}` });
                                                return;
                                            }
                                            // wait briefly and try to discover models
                                            await new Promise(r => setTimeout(r, 2000));
                                            try {
                                                const models = await ai.getAvailableModels();
                                                this._panel.webview.postMessage({ command: 'ollamaModels', success: true, models });
                                            } catch (e: any) {
                                                this._panel.webview.postMessage({ command: 'ollamaModels', success: false, message: e instanceof Error ? e.message : String(e) });
                                            }
                                        });
                                    } catch (e: any) {
                                        this._panel.webview.postMessage({ command: 'ollamaModels', success: false, message: e.message });
                                    }
                                } else {
                                    const models = await ai.getAvailableModels();
                                    this._panel.webview.postMessage({ command: 'ollamaModels', success: true, models });
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="form-group">
                ...
                <select id="aiModel" onchange="updateModelInfo()">
                    ...
                    <optgroup label="Other / Local Models">
                        <option value="minimax/minimax">Minimax (Minimax)</option>
                        <option value="kimi/kimi-k2">Kimi K2 (Kimi)</option>
                        <option value="openrouter/openrouter">OpenRouter (requires API key)</option>
                        <option value="ollama/llama2">Ollama (Local - llama2)</option>
                    </optgroup>
                </select>
            ...
            <div class="form-group" id="apiKeyGroup">
                <label for="aiApiKey">API Key (for selected model)</label>
                <p class="description">Enter the API key for the AI model you selected. Your API key is stored locally and never shared.</p>
            ...
            <div id="ollamaConfig" class="form-group" style="display:none; margin-top:8px;">
                <label for="ollamaBaseUrl">Ollama Base URL</label>
                <input type="text" id="ollamaBaseUrl" value="http://localhost:11434" placeholder="http://localhost:11434" style="width:100%; margin-top:6px;" />
                <div style="display:flex; gap:8px; margin-top:8px;">
                    <select id="ollamaModel" style="flex:1;"></select>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            apiKeyInput.addEventListener('blur', () => {
                if (apiKeyInput.value && apiKeyInput.value.length > 5 && document.getElementById('aiProvider').value === 'openrouter') {
                    testConnection();
                }
            });
            ...
            window.refreshOllamaModels = function() {
                const base = document.getElementById('ollamaBaseUrl').value || 'http://localhost:11434';
                vscode.postMessage({ command: 'fetchOllamaModels', baseUrl: base });
            };
        });
        ...
        window.addEventListener('message', event => {
            ...
            switch (message.command) {
                ...
                case 'connectionTest':
                    ...
                    break;
                case 'ollamaModels':
                    // Populate the Ollama model select
        ...
        function loadSettings(settings) {
            ...
            // Set provider and show appropriate UI
        ...
        function testConnection() {
            ...
            document.getElementById('connectionStatus').textContent = 'Testing connection to ' + provider + '...';
