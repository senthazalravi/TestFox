...
Write-Host "`nStep 1: Packaging extension..." -ForegroundColor Yellow
npm run package
...
Write-Host "`nStep 2: Publishing to VS Code Marketplace..." -ForegroundColor Yellow
...
if (-not $vsceToken) {
    Write-Host "VSCE_PAT not set, skipping VS Code Marketplace publish" -ForegroundColor Yellow
...
Write-Host "`nStep 3: Publishing to OpenVSX..." -ForegroundColor Yellow

# Load token from environment variable
$token = $env:OVSX_PAT
if (-not $token) {
    Write-Host "OVSX_PAT not set, checking user environment..." -ForegroundColor Yellow
